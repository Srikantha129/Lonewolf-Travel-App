...
class RecommendedForCity extends ConsumerWidget {
  final String city;

  const RecommendedForCity({
    super.key,
    required this.city,
  });
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    debugPrint('City passed to RecommendedForCity: $city');
        ...
        children: [
          ...
          hotelStream.when(
            data: (hotels) {
              ...
              final matchingHotels = hotels.where((hotel) => hotel.city.toLowerCase() == city.toLowerCase()).toList();

              // Determine the list to display:
              // - If there are matching hotels, take up to the first 10.
              // - If no matching hotels, take the first 10 from the full list.
              final recommendedList = matchingHotels.length >= 10
                  ? matchingHotels.take(10).toList()
                  : matchingHotels.isNotEmpty
                  ? matchingHotels
...
String getPrice(dynamic item) {
  if (item.avDates != null && item.avDates.isNotEmpty && item.avDates.first.values.isNotEmpty) {
    return '\$${item.avDates.first.values.first}';
  }
  return '\$70';
}