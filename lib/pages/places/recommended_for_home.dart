...
class RecommendedForHome extends ConsumerWidget {
  final String searchQuery; // Add this field

  const RecommendedForHome({
    super.key,
          ...
          hotelStream.when(
            data: (hotels) {
              ...
              final filteredHotels = searchQuery.isEmpty
                  ? hotels.take(10).toList() // Show first 10 hotels if no search
                  : hotels.where((hotel) =>
              hotel.name.toLowerCase().contains(searchQuery.toLowerCase()) ||
                  hotel.city.toLowerCase().contains(searchQuery.toLowerCase())).toList();

              // Determine the list to display:
              return filteredHotels.isEmpty
                  ? Center(child: Text('No hotels match your search', style: greyNormalTextStyle))
                  : ColumnBuilder(
