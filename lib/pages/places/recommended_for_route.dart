...
import 'package:lonewolf/pages/hotel/hotel_room_in_route.dart';
...
class RecommendedForRoute extends ConsumerWidget {
  const RecommendedForRoute({super.key});
...
class RecommendedForRoute extends ConsumerStatefulWidget {
  const RecommendedForRoute({super.key});

  @override
  ConsumerState<RecommendedForRoute> createState() => _RecommendedForRouteState();
}

class _RecommendedForRouteState extends ConsumerState<RecommendedForRoute> {
          ...
          hotelStream.when(
            data: (hotels) {
              ...
              final filteredHotels = hotels
                ...
                print('Searching "${hotelName}" and "${hotelCity}" for query "$trimmedQuery"');
                return hotelName.contains(trimmedQuery) || hotelCity.contains(trimmedQuery);
              })
                  .toList();
              ...
              final recommendedList = filteredHotels.take(10).toList(); // Limit to 10 hotels
              return ColumnBuilder(
                itemCount: recommendedList.length,
                mainAxisAlignment: MainAxisAlignment.start,
                mainAxisSize: MainAxisSize.max,
                              ...
                              children: [
                                ...
                                SizedBox(
                                  ...
                                  child: Column(
                                    ...
                                    children: [
                                      Text(
                                        '\$${(item.avDates?.isNotEmpty ?? false) ? item.avDates?.first.values.first : '70'}',  // Access the first value in the first map (price)
                                        style: bigPriceTextStyle,
                                        textAlign: TextAlign.center, // Align text properly
                                        overflow: TextOverflow.ellipsis, // Prevent overflow
                                      ),
  ...
  ratingBar(number) {
    return Row(
      ...
      crossAxisAlignment: CrossAxisAlignment.center,
      children: [
        Icon((number >= 1) ? Icons.star : Icons.star_border, color: Colors.lime[600]),
        Icon((number >= 2) ? Icons.star : Icons.star_border, color: Colors.lime[600]),
        Icon((number >= 3) ? Icons.star : Icons.star_border, color: Colors.lime[600]),
        Icon((number >= 4) ? Icons.star : Icons.star_border, color: Colors.lime[600]),
        Icon((number >= 5) ? Icons.star : Icons.star_border, color: Colors.lime[600]),
      ],
    );
  }
}
